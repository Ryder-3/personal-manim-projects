...
if TYPE_CHECKING:
    ...
    CellKind = Literal["text", "tex"]
    CellType = Union[str, tuple[CellKind, str], VMobject]
    TableDataType = Sequence[Sequence[CellType]]


class Table(VGroup):
    """
    A grid of cell entries arranged in rows and columns.

    Cell types:
    - ``str``: plain text via :class:`Text`, or LaTeX via :class:`Tex` when the
      string looks like math (contains ``\\`` or ``$``)
    - ``("tex", "...")`` / ``("text", "...")``: force LaTeX or Text
    - :class:`VMobject`: used as-is

    Config keys apply by row/cell type (not merged across roles):
    - ``header_config``: entire first row when ``header_row=True`` (Text and Tex)
    - ``tex_config``: LaTeX cells in non-header rows
    - ``element_config``: Text cells in non-header rows

    Use ``full_width=True`` or ``max_width=...`` to span the frame; long plain
    strings wrap via Pango's ``line_width`` (LaTeX cells are not wrapped).

    Example::

        table = Table(
            [
                ["Claim", ("tex", r"$\widehat{p}$")],
                ["A long claim ...", "24%"],
            ],
            header_row=True,
            full_width=True,
            column_width_ratios=(4, 1),
            header_config=dict(scale=0.6),
            tex_config=dict(scale=0.5),
        )
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.tex_config = tex_config or dict()
    ...
    def _cell_config(self, row_index: int, *, for_tex: bool = False) -> dict:
        """Header row uses header_config; body uses tex_config or element_config."""
        if self.header_row and row_index == 0:
            return dict(self.header_config)
        if for_tex:
            return dict(self.tex_config)
        return dict(self.element_config)
    ...
    @staticmethod
    def _normalize_latex_string(value: str) -> str:
        """Turn mixed strings like ``$\\hat{p}$ (NWS)`` into valid Tex."""
        value = value.strip()
        if "$" not in value:
            if Table._looks_like_latex(value) and re.search(
                r"(?<![\\{])[A-Za-z]{2,}", value
            ):
                return rf"\text{{{value}}}"
            return value
        ...
        for index, part in enumerate(parts):
            ...
            if index % 2 == 0:
                text = part.strip()
                if text:
                    pieces.append(rf" \text{{{text}}} ")
            else:
                pieces.append(part.strip())
        return "".join(pieces)

    @staticmethod
    def _apply_scale(mob: VMobject, scale: float | None) -> VMobject:
        if scale is not None:
            mob.scale(scale)
        return mob

    def _make_tex_mobject(self, value: str, **config) -> VMobject:
        latex = self._normalize_latex_string(value)
        scale = config.pop("scale", None)
        mob = Tex(latex, **config)
        return self._apply_scale(mob, scale)

    def _make_text_mobject(self, value: str, **config) -> VMobject:
        scale = config.pop("scale", None)
        mob = Text(value, **config)
        return self._apply_scale(mob, scale)

    def _parse_cell(self, element: CellType) -> tuple[str, str | None]:
        """
        Returns (kind, content) where kind is ``text``, ``tex``, or ``mobject``.
        """
        if isinstance(element, VMobject):
            return "mobject", element
        if isinstance(element, tuple) and len(element) == 2:
            kind, content = element
            if kind not in ("text", "tex"):
                raise ValueError(
                    f"Unknown cell kind {kind!r}; use 'text' or 'tex'"
                )
            return kind, str(content)
        value = str(element)
        if self._looks_like_latex(value):
            return "tex", value
        return "text", value
    ...
    def _element_to_mobject(
        ...
        kind, content = self._parse_cell(element)
        if kind == "mobject":
            return content
        if kind == "tex":
            return self._make_tex_mobject(
                content,
                **self._cell_config(row_index, for_tex=True),
            )
        text_config = self._cell_config(row_index, for_tex=False)
        if line_width is not None:
            text_config["line_width"] = line_width
        return self._make_text_mobject(content, **text_config)
        ...
        for i, row in enumerate(self.table_data):
            for j, element in enumerate(row):
                if isinstance(element, VMobject):
                    ...
                    cell = self._element_to_mobject(element, row_index=i)
    ...
    def _create_uniform_mobject_matrix(self) -> list[list[VMobject]]:
        ...
        for i, row in enumerate(self.table_data):
            mob_matrix.append([
                self._element_to_mobject(element, row_index=i)
                for element in row
            ])
